...
module.exports = (() => {
    ...
    return {
        game: () => {
            if (!tagpro.tools.settings.macros) {
                return;
            }
            tagpro.tools.settings.macros.forEach(macro => {
                tagpro.tools.global.shortcuts.macros.set(
                    macro.key_code, macro.toAll, macro.message, macro.options
                );
            });
        },
        menu: () => {
                ...
                tagpro.tools.global.menu.setSection('Macros', (() => {
                    let tools_settings = document.createElement('div'),
                        ...
                        update_macros = () => {
                            ...
                            for (let i = 0; i < document.getElementsByClassName('macro-data').length; i++) {
                                if (document.getElementsByClassName('macro-data')[i]
                                        .getElementsByTagName('input').length !== 6) {
                                    continue;
                                }
                                if (!parseInt(document.getElementsByClassName('macro-data')[i]
                                        .getElementsByTagName('input')[0].id, 10)) {
                                    continue;
                                }
                                let new_macro = {},
                            ...
                            let newInput = (value, position) => {
                                ...
                                case 0:
                                    ...
                                    new_input.addEventListener('keydown', (event) => {
                                        ...
                                        event.target.value = niceKeyCodes[event.keyCode] ||
                    ...
                    try {
                        for (let macro of tagpro.tools.settings.macros) {
                            tools_settings.appendChild(
                                new MacroMenu(macro.key_code, macro.toAll, macro.message, macro.options).toDom()
                            );
                        }
                    } catch (e) {
