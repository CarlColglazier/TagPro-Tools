...
module.exports = (() => {
    ...
    return {
        game: function () {
            ...
            let small_font = 'bold 9pt ' + tagpro.tools.settings.values.primary,
                ...
                return new PIXI.Text(text,
                    {
                        ...
                        fill: color || WHITE,
                        stroke: tagpro.tools.settings.values.stroke || BLACK,
            ...
            tagpro.renderer.updateTagpro = (player) => {
                ...
                if (player.tagpro) {
                    if (!player.sprites.tagproTint) {
                        let tint = player.sprites.tagproTint = new PIXI.Graphics(),
                            color = parseInt(tagpro.tools.settings
                                .values.green.replace('#', ''), 16);
                        tint.beginFill(color || 0x00FF00, 0.35)
                            .lineStyle(3, color || 0x00FF00)
                            .drawCircle(20, 20, 20);
                    ...
                    if (player.sprites.tagproSparks) {
                        ...
                        let sparksIndex = tr.emitters.indexOf(player.sprites.tagproSparks);
            ...
            tr.updateRollingBomb = function (player) {
                ...
                if (player.bomb) {
                    if (!player.sprites.bomb) {
                        if (!tr.options.disableParticles) {
                            player.sprites.rollingBomb = new cloudkid.Emitter(player.sprites.ball,
                                [tr.particleTexture],
                        ...
                        var bomb = player.sprites.bomb = new PIXI.Graphics(),
                            color = parseInt(tagpro.tools.settings
                                .values.yellow.replace('#', ''), 16);
                        bomb.beginFill(color, 0.75).drawCircle(20, 20, 20);
                        player.sprites.ball.addChild(bomb);
                    } else {
                        player.sprites.bomb.alpha = Math.abs(
                            0.75 * Math.sin(performance.now() / 150)
                        );
                    }
                } else {
                    if (player.sprites.bomb) {
            ...
            tagpro.ui.scores = function () {
                ...
                let n = e + (tagpro.score.r ? tagpro.score.r.toString() : '0'),
                ...
                if (tagpro.ui.sprites.redScore) {
                    ...
                    tagpro.ui.sprites.redScore = new PIXI.Text(n, {
                        fill: tagpro.tools.settings.values.red,
                        font: big_font
                    });
            ...
            function updatePlayerText() {
                ...
                // Text colors!
                tagpro.renderer.drawName = (player) => {
                    if (!player.sprites.name ||
                        player.sprites.name.text !== player.name ||
                        ...
                        // Fixes a bug caused when the object is false and then is turned
                        // true after the text is rendered.
                        player.sprites.name.auth = player.auth;
            ...
            wait(playerCount(), setTimeout(updatePlayerText, 100));
        }
    };
})();
