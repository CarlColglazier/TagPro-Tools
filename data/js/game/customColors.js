...
tagpro.tools.optional.customColors = function() {
    ...
    var small_font = "bold 9pt " + tagpro.tools.settings.ui.fonts.primary,
        big_font = "bold 48pt " + tagpro.tools.settings.ui.fonts.primary,
        timer_font = "bold 30pt " + tagpro.tools.settings.ui.fonts.primary;

    function fixChatNameColors(node) {
        if (!node.style) {
            return;
        }
        node.style.fontFamily = tagpro.tools.settings.ui.fonts.primary;
        if (node.style.color) {
            switch (node.style.color) {
            case "rgb(255, 181, 189)":
                node.style.color = (tagpro.tools.settings.ui.colors["light red"]) ?
                    tagpro.tools.settings.ui.colors["light red"] :
                    (tagpro.tools.settings.ui.colors.red) ?
                        tagpro.tools.settings.ui.colors.red :
                        node.style.color;
                break;
            case "rgb(207, 207, 255)":
                node.style.color = (tagpro.tools.settings.ui.colors["light blue"]) ?
                    tagpro.tools.settings.ui.colors["light blue"] :
                    (tagpro.tools.settings.ui.colors.blue) ?
                        tagpro.tools.settings.ui.colors.blue :
                        node.style.color;
                break;
            default:
    ...
    tagpro.renderer.prettyText = function (text, color) {
        ...
        if (color) {
            ...
            case "#bfff00":
                color = tagpro.tools.settings.ui.colors.green;
                break;
            default:
                break;
            }
        ...
        return new PIXI.Text(text,
            {
                ...
                fill: color || "#FFFFFF",
                stroke: tagpro.tools.settings.ui.colors.stroke || "#000000",
                strokeThickness: tagpro.tools.settings.ui.fonts["stroke thickness"] || 3
            });
    };
    ...
    tr.largeText = function (text, color) {
        ...
        if (color) {
            switch (color) {
            case '#0000ff':
                color = tagpro.tools.settings.ui.colors.blue;
                break;
            case '#ff0000':
                color = tagpro.tools.settings.ui.colors.red;
                break;
            default:
                break;
            }
         ...
         return new PIXI.Text(text,
            {
                ...
                fill: color || "#ffffff",
                stroke: tagpro.tools.settings.ui.colors.stroke || "#000000",
                strokeThickness: tagpro.tools.settings.ui.fonts["stroke thickness"] || 2
            });
    };
        ...
        if (player.tagpro) {
            if (!player.sprites.tagproTint) {
                var tint = player.sprites.tagproTint = new PIXI.Graphics(),
                    color = parseInt(tagpro.tools.settings.ui.colors.green.replace('#', ''), 16);
        ...
        if (player.bomb) {
            if (!player.sprites.bomb) {
                ...
                var bomb = player.sprites.bomb = new PIXI.Graphics(),
                    color = parseInt(tagpro.tools.settings.ui.colors.yellow.replace('#', ''), 16);
    ...
    tagpro.ui.scores = function () {
        ...
        if (tagpro.ui.sprites.redScore) {
            ...
            tagpro.ui.sprites.redScore = new PIXI.Text(n, {fill: tagpro.tools.settings.ui.colors.red, font: big_font});
            tagpro.ui.sprites.blueScore = new PIXI.Text(r, {
                fill: tagpro.tools.settings.ui.colors.blue,
                font: big_font
            });
